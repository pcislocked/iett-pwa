...
- **Accessibility (A11y) & UX Polish:**
  - Fixed focus trap bugs and added dialog/modal ARIA roles to all overlay modals.
  - Resolved keyboard navigation trap inside Search Page dropdown autocompletes.
  - Implemented automatic animation disable on leaflet maps when `prefers-reduced-motion` is enabled.
